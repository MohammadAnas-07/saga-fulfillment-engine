 ...
 * still tell the two apart: a saga that refunded real money and a saga that never took
 * any are equally finished, but they are not the same story.
 *
 * <p>inventory-service declares an identically named and valued enum, so a consumer can
...
public enum CompensationOutcome {
    ...
    /**
     * There was nothing to undo. Not an error: the saga may have failed before payment
     * ran, the payment itself may have failed, or compensation may have already run.
     */
    NOTHING_TO_REVERSE
}
